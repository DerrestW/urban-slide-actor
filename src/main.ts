...
Actor.main(async () => {
  ...
  for (const city of cities) {
    ...
    const queueName=`${city.city}-${city.stateCode}-${Date.now()}`.toLowerCase().replace(/[^a-z0-9-]+/g,"-").replace(/-+/g,"-").replace(/^-+|-+$/g,""); log.info(`Opening queue: ${queueName}`); const requestQueue = await RequestQueue.open(queueName);
